...
module Bench (Hash : Hash.S) = struct
  module%bench [@name_suffix Hash.description] _ = struct
    ...
    let rec dn n =
      if n <= 0
      then []
      else (n, String.init n ~f:(fun i -> if i mod 2 = 0 then 'j' else 's')) :: dn (n - 1)
    ;;
...
module Bench_hashtbl_hash = struct
  (* This module is a verbatim copy of the above, except that we use Hashtbl.hash every
     where. *)
  module%bench [@name "Hashtbl.hash"] _ = struct
    type a = int
    type b = a * float
...
module%bench _ = Bench (Traverse_only)
module%bench _ = Bench (Check_initialized_correctly)
module%bench _ = Bench (Base.Hash)
module%bench _ = Bench (Siphash_lib.Siphash)
module%bench _ = Bench (Ppx_hash_runtime_test.Perfect_hash)
